...
public class df {
    ...
    public static void createTable(String tableName,String... dd) throws IOException {
        if (dd.length<=0){
        ...
        for (String ww : dd) {
            HColumnDescriptor hColumnDescriptor = new HColumnDescriptor(ww);
    ...
    public static void get(String tableName,String rowKey,String a,String b) throws IOException {
        Table table = connection.getTable(TableName.valueOf(tableName));
        Get get = new Get(Bytes.toBytes(rowKey));
        get.addColumn(Bytes.toBytes(a),Bytes.toBytes(b));
    ...
    public static void scan(String tableName,String a,String b) throws IOException {
        //获取表对象
        Table table = connection.getTable(TableName.valueOf(tableName));
        Scan scan = new Scan();
        scan.addColumn(Bytes.toBytes(a),Bytes.toBytes(b));
    ...
    public static void main(String[] args) throws IOException {
        ...
        System.out.println(isTableExist("emp17124080208"));
        System.out.println("--------------创建表--------------");
        createTable( "emp17124080208","member_id","address","info");
        System.out.println("创建表是否成功"+isTableExist("emp17124080208"));
        insert("Rain","emp17124080208",column1,value1,column2,value2,column3,value3);
        System.out.println("---------获取info列族信息---------");
        get("emp17124080208","Rain","info","age");
        get("emp17124080208","Rain","info","birthday");
        get("emp17124080208","Rain","info","industry");
        System.out.println("----查看扫描info:birthday列数据---");
        scan("emp17124080208","info","birthday");
        System.out.println("-----------查看全部数据-----------");
        scanall("emp17124080208");
        close();
    }
}
